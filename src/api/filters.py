...
async def create_group(
        data: schemas.FilterGroupCreate,
...
async def list_groups(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(models.FilterGroup)
        .options(
            selectinload(models.FilterGroup.keywords)
        )
    )
...
async def add_keyword(
        data: schemas.FilterKeywordCreate,
        db: AsyncSession = Depends(get_db)
):
    keyword = models.FilterKeyword(
...
@router.post("/minus-keywords/bulk", status_code=201)
async def add_minus_keywords_bulk(
        data: schemas.FilterKeywordMinusBulkCreate,
        db: AsyncSession = Depends(get_db),
):
    keywords = [
